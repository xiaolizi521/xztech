...
$Config = {
	...
	ks_domain	=> "kslan.dmy0.dev.peer1.com",
	ks_host		=> "ks1.dmy0.dev.peer1.com",
	ks_ipaddr	=> "10.6.0.2",
	ks_public_ipaddr	=> "70.33.255.58",
	ks_public_network	=> "70.33.255.0",
	ks_public_netmask	=> "255.255.255.192",
	ks_public_gateway	=> "70.33.255.1",
    ...
    lwp_agent	=> "SBKS/0.1 ",
    bootServerMacs => [ "00:0d:61:80:7d:66" ]
};
...
if (($Config->{net_www_user}) && ($Config->{net_www_pass})) {
    $Config->{net_baseurl} = sprintf("https://%s:%s@%s",
    $Config->{net_www_user}, $Config->{net_www_pass}, $Config->{net_www_host});
}
else {
    $Config->{net_baseurl} = sprintf("https://%s", $Config->{net_www_host});
}
