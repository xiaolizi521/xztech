sub bbu_check() {
        ...
        $bbu =~ s/\s+$//;
        if (($bbu eq "Charging" || $bbu eq "Yes" || $bbu eq "OK")) {
...
# Caleb Collins (11-24-08) added new
# section for port/model detection
#
#        $_[0]->{"3ware"} = "9650";
...
#        print $_[0]->{"3ware"} . "\n";
        ...
        $raid_model =~ s/\s+$//;
        if ($raid_model eq "9650SE-2LP") {
                $_[0]->{"raid_controller"} = "9650SE-2LP";
                $_[0]->{"raid_controller_hash"} = &raid_hash("9650SE-2LP");
                $_[0]->{"raid_controller_ports"} = "2";
                print "3ware 9650SE-2LP card detected: ";
                print $_[0]->{"raid_controller"} . "\n";
                print $_[0]->{"raid_controller_hash"} . "\n";
                print $_[0]->{"raid_controller_ports"} . "\n";
                &bbu_check;
        }
        elsif ($raid_model eq "9650SE-4LPML") {
                $_[0]->{"raid_controller"} = "9650SE-4LPML";
                $_[0]->{"raid_controller_hash"} = &raid_hash("9650SE-4LPML");
                $_[0]->{"raid_controller_ports"} = "4";
                print "3ware 9650SE-4LPML card detected: ";
                print $_[0]->{"raid_controller"} . "\n";
                print $_[0]->{"raid_controller_hash"} . "\n";
                print $_[0]->{"raid_controller_ports"} . "\n";
                &bbu_check;
        }
        elsif ($raid_model eq "9650SE-8LPML") {
                $_[0]->{"raid_controller"} = "9650SE-8LPML";
                $_[0]->{"raid_controller_hash"} = &raid_hash("9650SE-8LPML");
                $_[0]->{"raid_controller_ports"} = "8";
                print "3ware 9650SE-8LPML card detected: ";
                print $_[0]->{"raid_controller"} . "\n";
                print $_[0]->{"raid_controller_hash"} . "\n";
                print $_[0]->{"raid_controller_ports"} . "\n";
                &bbu_check;
