<?
    ...
    $db->query( "SELECT * from messages
    LEFT OUTER JOIN client_plan ON messages.project_id = client_plan.id
    LEFT OUTER JOIN message_recipients ON messages.id = message_recipients.message_id
    LEFT OUTER JOIN plan_classification ON client_plan.classification_id = plan_classification.id
    WHERE message_recipients.employee_id = ".$employeeArray['id']."
    ORDER BY messages.date DESC" );
	...
	<tr>
		...
		<td>
			<? if( $row['messages.project_id'] > 0 ) { ?>
				<a href="client_manage.php?id=<?= $row['client_plan.id'] ?>"><?= $row['plan_classification.name'] ?></a>
			<? }else{ ?>
				No Project Selected
			<? }?>
		</td>
