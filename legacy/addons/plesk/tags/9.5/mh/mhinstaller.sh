...
PPASS=$RPASS
HOSTNAME="$HOST.pubip.peer1.net"

# We need to set the hostname explicitly before setup continues.
# First, set the hostname via the hostname command
sethost=$(hostname ${HOSTNAME})

# Next, we're going to overwrite the network file, and then append to it.

cat /dev/null > /etc/sysconfig/network

# Now, we are going to write the new network file
echo "### Begin /etc/sysconfig/network" >> /etc/sysconfig/network
echo "#" >> /etc/sysconfig/network
echo "NETWORKING=yes" >> /etc/sysconfig/network
echo "NETWORKING_IPV6=no" >> /etc/sysconfig/network
echo "HOSTNAME=${HOSTNAME}" >> /etc/sysconfig/network

# Now, we are going to rewrite the hosts file.
...
echo "${IPADDR}     ${HOST} ${HOSTNAME}" >> /etc/hosts
...
postlog() {                                                    
	...
	if [ -z "$status" ] ; then status="ksfail" ; fi
        ...
        echo "$logstring" >> /tmp/plesk95/install.log
}
...
yum remove -y courier-imap courier-authlib vpopmail imp horde qmail-pop3d squirrelmail qmail bind-chroot dovecot vsftpd
...
# Added this to fix that debian does not support sitebuilder. This should prevent installer from stopping.

if [ -s /etc/debian_version ]; then
    postlog "INFO" "Debian does not support sitebuilder. Skipping this step..."
else
    while [ "$loop" -lt 5 ] && [ "$ec" -ne 0 ]; do
        # If this is a loop, sleep for a bit. Exponentially waits longer per retry
        # to try and let the repos fix themselves
...
# Now we need to create the IPTables rule for plesk
(
cat <<'EOF'
### Begin /etc/sysconfig/iptables.d/90psa
#
*filter
...
COMMIT
EOF
) > /etc/sysconfig/iptables.d/90psa

# Restart IPTables to reflect these new rules

/sbin/service iptables stop
/sbin/service iptables start

# Fix permissions on named.conf
...
mysql -uadmin -p`cat /etc/psa/.psa.shadow` --execute="DELETE FROM psa.IP_Addresses WHERE id = 1"
mysql -uadmin -p`cat /etc/psa/.psa.shadow` --execute="DELETE FROM psa.ip_pool WHERE ip_address_id = 1"
