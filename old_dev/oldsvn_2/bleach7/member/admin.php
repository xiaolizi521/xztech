...
if ( isset ( $_POST['add_news'] ) ) {
	...
	if ( count ( $errors ) == 0 ) {
		...
		$query 		= sprintf( "INSERT INTO `news` ( `id`, `headline`, `poster`, `news` ) VALUES ( %s, '%s', '%s', '%s' )",
						$nid,
						$headline,
						$user_info['username'],
						$news ); 
...
if ( isset ( $_POST['edit_news'] ) ) {
	...
	if ( count ( $errors ) == 0 ) {
		$headline 	= mysql_real_escape_string ( htmlspecialchars ( $_POST[headline], ENT_QUOTES ) );
		$news 		= mysql_real_escape_string ( $_POST[news] );
		$query 		= sprintf( "UPDATE `news` SET `headline`='%s', `news`='%s' WHERE `id`=%s",
							$headline,
							$news,
							$id );
		$database->query( $query );
			...
			if ( isset ( $id ) && !empty ( $id ) ) { //editting news with an item selected
				...
				if ( $user_info['type'] == "20" || $user_info['type'] == "21" || $user_info['type'] == "30" || 
					...
					$query = sprintf( "SELECT * FROM `news` WHERE `id`=%s",
									$id );
					$result_edit_id = $database->query( $query );
					//$result_edit_id = mysql_query( "SELECT * FROM news WHERE id='$id'" );
				}
				//$edit_id = mysql_fetch_array( $result_edit_id );
				if ( $result_edit_id->num_rows ) { //a valid news id is found
					$edit_id = $result_edit_id->fetch_array();
					if ( count ( $errors ) > 0 ) {
						echo "<table cellpadding='0' cellspacing='0' class='main'>
							<tr>
								<td>";
...
";
				...
				if ( $user_info['type'] == "20" || $user_info['type'] == "21" || $user_info['type'] == "30" || $user_info['type'] == "31" 
					...
					$result_edit = $database->query( "SELECT * FROM news ORDER BY id DESC" );
				}
				$count = 1;
				...
				while ( $edit = $result_edit->fetch_array() )
			...
			if ( isset ( $id ) && !empty ( $id ) ) { //delete news with an item selected
				...
				$query = sprintf( "SELECT `id`, `poster` FROM `news` WHERE `id`=%s AND `poster`='%s'",
								$delete_id,
								$delete_username );
