...
$result_news = mysql_query( "SELECT * FROM news ORDER BY id DESC LIMIT $start, $display" )
...
// Make the links to other pages, if necessary.
		if ($num_pages > 1) {
			...
			if ($current_page != 1) {
				echo '<a href="?page=member/newstest&&s=' . ($start - $display) . '&np=' . $num_pages . '">Previous</a> ';
			}

			// Make all the numbered pages.
			for ($i = 1; $i <= $num_pages; $i++) {
				if ($i != $current_page) {
					echo '<a href="?page=member/newstest&&s=' . (($display * ($i - 1))) . '&np=' . $num_pages . '">' . $i . '</a> ';
				} else {
					echo $i . ' ';
				}
			}
			...
			if ($current_page != $num_pages) {
				echo '<a href="?page=member/newstest&&s=' . ($start + $display) . '&np=' . $num_pages . '">Next</a>';
			}
