...
$result_news = mysql_query ( 'SELECT * FROM `news` WHERE `id`=\'' . mysql_real_escape_string ( $id ) . '\'' );
...
if ( mysql_num_rows ( $result_news ) == 0 ) {
	...
	while ( $show_news = mysql_fetch_array ( $result_news ) ) {
		...
		$news = htmlentities  ( $news, ENT_QUOTES ); // Removed, ISO-8859-15
...
function CommentOption( $id, $username, $type ) {
	global $site_path;
	if ( $type == 'delete' ) {
		$type_url = '<a href="'.$site_path.'/comments&amp;id=' . $_GET['id'] . '&amp;pg=' . $_GET['pg'] . '&amp;' . $type . '=' . $id . ',' . $username . '" onclick="javascript:DeleteComment (\'' . $site_path . '/comments&amp;id=' . $_GET['id'] . '&amp;pg=' . $_GET['pg'] . '&amp;' . $type . '=' . $id . ',' . $username . '\')">' . ucwords( $type ) . '</a>';
	}elseif ( $type == 'ban' ){
...
if ( isset ( $_POST['submit_comment'] ) ) {
	if ( !isbanned ( $user_info['user_id'] ) ) {
		...
		$update_comment_num = mysql_query ( 'UPDATE `news` SET `comments` = ( `comments` + 1 ) WHERE `id`=\'' . mysql_real_escape_string ( $id ) . '\'' );
...
if ( isset ( $_POST['edit_comment'] ) ) {
	...
	$update_comment = mysql_query ( "UPDATE news_comments SET comment='$comment' WHERE newsid='$id' AND id='$edit_id'" );
	echo "<script>document.location='index.php?page=member/comments&id=$id&pg=$pg'</script>";
...
if ( isset ( $_GET['delete'] ) && !empty ( $_GET['delete'] ) ) {
	...
	$result_delete_post = mysql_query ( 'SELECT `news_comments`.`id`, `news_comments`.`newsid`, `news_comments`.`comment`, `users`.`username` FROM `news_comments` LEFT JOIN `users` ON ( `news_comments`.`poster` = `users`.`username` ) WHERE `news_comments`.`newsid` = \'' . mysql_real_escape_string ( $id ) . '\' AND `news_comments`.`id` = \'' . mysql_real_escape_string ( $delete_id ) . '\' AND `users`.`username` = \'' . mysql_real_escape_string ( $delete_username ) . '\'' );
	if ( mysql_num_rows ( $result_delete_post ) > 0 ) {
		$delete_post = mysql_query ( 'DELETE FROM `news_comments` WHERE `newsid` = \'' . mysql_real_escape_string ( $id ) . '\' AND `id` = \'' . mysql_real_escape_string ( $delete_id ) . '\'' );
...
if ( $comments_num == 0 ) {
	...
	$result_comments = mysql_query ( 'SELECT `news_comments`.*, `users`.* FROM `news_comments` LEFT JOIN `users` ON ( `users`.`username` = `news_comments`.`poster`) WHERE `news_comments`.`newsid`=\'' . mysql_real_escape_string ( $id ) .'\' ORDER BY `news_comments`.`id` ASC LIMIT ' . mysql_real_escape_string ( $offset ) . ', ' . mysql_real_escape_string ( $limit ) );
...
if ( $edit != 'null' && !empty ( $edit ) ) {
	list ( $edit_id, $edit_username ) = explode ( ",", $edit );
	$result_edit_post = mysql_query ( 'SELECT `news_comments`.`id`, `news_comments`.`newsid`, 			`news_comments`.`comment`, `users`.`username` FROM `news_comments` LEFT JOIN `users` ON ( `news_comments`.`poster` = `users`.`username` ) WHERE `news_comments`.`newsid` = \'' . mysql_real_escape_string ( $id ) . '\' AND `news_comments`.`id` = \'' . mysql_real_escape_string ( $edit_id ) . '\' AND `users`.`username` = \'' . mysql_real_escape_string ( $edit_username ) . '\'' );
...
if ( $quote != 'null' && !empty ( $quote ) ) {
	...
	$result_quote_post = mysql_query ( 'SELECT `news_comments`.`id`, `news_comments`.`newsid`, `news_comments`.`comment`, `users`.`username` FROM `news_comments` LEFT JOIN `users` ON ( `news_comments`.`poster` = `users`.`username` ) WHERE `news_comments`.`newsid` = \'' . mysql_real_escape_string ( $id ) . '\' AND `news_comments`.`id` = \'' . mysql_real_escape_string ( $quote_id ) . '\' AND `users`.`username` = \'' . mysql_real_escape_string ( $quote_username ) . '\'' );
