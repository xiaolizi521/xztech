...
$rss = '<?xml version="1.0" encoding="utf-8" standalone="yes"?>
	...
	<channel>
		<title>Bleach7 Wallpapers RSS Feed</title>
...
$result_gallery = mysql_query( 'SELECT * FROM `gallery` WHERE `approved` = \'1\' AND `category`=\'2\' ORDER BY `id` DESC LIMIT 0, 153' )
...
while ( $gallery = mysql_fetch_array ( $result_gallery ) ) {
	...
	$link = 'http://www.bleach7.com/?page=media/wallpaperview&amp;id='.$id;
...
';
$rss .= '</rss>';
( $fp = fopen ( '/home/bleach7/public_html/rss_wallpaper.xml', 'w' ) ) or die ( 'couldn\'t open' );
fwrite ( $fp, $rss );
fclose ( $fp );
?>