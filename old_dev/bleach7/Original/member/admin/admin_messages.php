<?php
	...
	echo "<form method='post' name='form_messages'>";
		...
		<br />";
		$result_message_list = mysql_query( "SELECT * FROM admin_message WHERE rank <= $user_rank ORDER BY id DESC" );
		if(!isset($result_message_list)) {
		if ( mysql_num_rows ( $result_message_list ) > 0 ) {
			if ( count ( $errors ) > 1 ) {
			...
			echo "Currently there are either no messages posted, or none that are available for you to see.
			";
		}
		}
	echo "</form>";
