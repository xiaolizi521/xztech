...
function getVLANName($vlan_id) {
    global $GLOBAL_db;
    $res = $GLOBAL_db->GetVal("select \"VLANNumber\" from \"ADDR_PrivateNetBlock\" where \"ADDR_PrivateNetBlockID\"=$vlan_id;");
    return $res;
}
...
function showVLANList($computer_number, $customer_number, $type) {
    ...
    if ($type == 'new') {
		...
		$computer->Init($customer_number,$computer_number,$GLOBAL_db);
		$vlan_list = $computer->getAllAggExNetList();
		$blah = 'Please select from list of NEW VLANs.';
    }


    ?>
    <form name="vlanform" action="popupConfigureAggExNet.php" method="POST">
    <table>
    <tr>
    <td>
    <fieldset>
    ...
    <select name="selected" size='10'>
        ...
        foreach($vlan_list as $vlan) {
            print "<option value=\"$vlan[id]\"> $vlan[name] </option>\n";
        }
        ?>
    </select>
    </fieldset>
...
function assignComputer($computer_number, $customer_number, $vlan_id) {
    ...
    print "<A HREF=/tools/popupConfigureAggExNet.php?customer_number=$customer_number&computer_number=$computer_number&vlan_id=$vlan_id&confirm=1 class='text_button'>Continue</A>";
...
<TABLE class="blueman">
<TR><TH class="blueman">Configure Computer # <?=$computer_number ?> for AggExNet</TH></TR>
<TR><TD class="blueman">
<P>
<?
    ...
    isset($vlan_id))
{
    ...
    $res = $GLOBAL_db->SubmitQuery("insert into xref_computer_aggexnet (computer_number, customer_number, vlan_id) values ($computer_number, $customer_number, $vlan_id)");
    $GLOBAL_db->CommitTransaction();
    print "Successfully configured computer for AggExNet!";
?>
