...
$s = mysql_fetch_assoc(mysql_query("SELECT * FROM `logging` WHERE `page` = 'sig'"));
...
$now = mktime();
$diff = $now - $time;
$diff = round($s['count'] / $diff,2);
echo "This works out to be $diff signature views every second!<br>";
