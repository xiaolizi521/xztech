...
{
	if($_GET['sort'] == 'roman') {
		$numbers = mysql_query ( "SELECT rname,series,directory,complete,latest,updated FROM toshokan WHERE hidden < 1 ORDER BY rname ASC" );
	...
	elseif($_GET['sort'] == 'english') {
		$numbers = mysql_query ( "SELECT ename,series,directory,complete,latest,updated FROM toshokan WHERE hidden < 1 ORDER BY ename ASC" );
