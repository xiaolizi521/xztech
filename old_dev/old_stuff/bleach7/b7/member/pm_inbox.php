...
';
				...
				echo '</td>
						<td style="width: 20%;"><a href="', $site_path, '/member&amp;id=', $pm['sent_by'], '"><b>', $pm['sent_by'], '</b></a></td>
						<td>', $date, '</td>
					</tr>
';
				$x++;
...
<table cellpadding="3" cellspacing="0" class="main" style="width: 100%;">
	...
	<tr>
		<td style="text-align: center;">
			<input type="submit" name="dv" value="Delete PM(s)" class="form" />
...
##########################################
	...
	if ( mysql_num_rows ( $result ) == 0 ) {
		...
		$pm_options = '<a href="' . $site_path . '/pm_compose&amp;to=' . $pm['sent_by'] . '&amp;reply=' . $pm['id'] . '">Reply</a> | <a href="'.$site_path.'/pm_inbox&id='.$id.'&dv=1" onclick="DeletePM()">Delete</a>';
		if ( empty ( $pm['avatar'] ) ) {
			$member_avatar = '<img src="' . $site_url . '/' . $script_folder . '/images/avatars/none.gif" alt="none" style="width: 100px; height: 100px" />';
		}
		else {
			list ( $avatar_width, $avatar_height ) = getimagesize ( $pm['avatar'] );
			if ( $avatar_width > 100 || $avatar_height > 100 ) {
				$member_avatar = '<img src="' . $pm['avatar'] . '" alt="' . $pm['username'] . '" style="width: 100px; height: 100px" />';
			}
			else {
...
';
	...
	echo '				<p style="text-align: center;">';?>
					<INPUT type="button" value="Back To Inbox" onClick="location.href='?page=member/pm_inbox'" class="form">
	<?
