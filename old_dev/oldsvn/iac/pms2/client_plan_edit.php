<?
	...
	if( sizeof( $post ) > 0 )
	{		
		$db->update( "client_plan" , array( "client_id" => $post['client_id'], "classification_id" => $post['plan_classification_id'], "rate_classification_id" => $post['rate_classification_id'], "notes" => $post['notes'], "status" => $post['status'], "retained_hours" => $post['retainedHours'], "hours_purchased" => $post['hoursPurchased'] ), array( "id" => $post['plan_id'] ));		
		header( "Location: client_activity.php" );
	}
	
	$planId = 0;
	$clientId = 0;
	$planClassificationId = 0;
	$rateClassificationId = 0;
	$notes = "";
	$status = "";
	$retainedHours = "";
	$hoursPurchased = "";
	...
	if ( sizeof( $request ) > 0 ){
		$planId = $request['id'];
		//$db->debug = true;
		$db->query( "SELECT * FROM client_plan WHERE id = $planId" );
		if( $db->result['rows'] > 0 ){
			while( $row = mysql_fetch_assoc( $db->result['ref'] ) ){
				$clientId = $row['client_id'];
				$planClassificationId = $row['classification_id'];
				$rateClassificationId = $row['rate_classification_id'];
				$notes = $row['notes'];
				$status = $row['status'];
				$retainedHours = $row['retained_hours'];
...
<div>
<h1>Edit Client Plan</h1>
<form action="client_plan_edit.php" method="post" name="task">
...
<table cellpadding="5">
	<tr>
		...
		<td>
			...
			<option value=""></option>
			<? while( $row = mysql_fetch_assoc( $db->result['ref'] ) ): ?>
			<option value="<?= $row['id'] ?>" <? if ( $row['id'] == $clientId ) echo " selected ";?> ><?= $row['organization'] ?></option>
			<? endwhile; ?>
			</select>
			<? else: ?>
	...
	<tr>
		...
		<td>
			...
			<option value=""></option>
			<? while( $row = mysql_fetch_assoc( $db->result['ref'] ) ): ?>
			<option value="<?= $row['id'] ?>" <? if ( $row['id'] == $planClassificationId ) echo " selected "; ?>><?= $row['name'] ?></option>
			<? endwhile; ?>
			</select>
			<? else: ?>
	...
	<tr>
		...
		<td>
			...
			<option value=""></option>
			<? while( $row = mysql_fetch_assoc( $db->result['ref'] ) ): ?>
			<option value="<?= $row['id'] ?>" <? if ( $row['id'] == $rateClassificationId ) echo " selected "; ?>><?= $row['name'] ?></option>
			<? endwhile; ?>
			</select>
			<? else: ?>
	...
	<tr id="retainedHours" name="retainedHours">
		...
		<td style="color:red;font-size:9pt;"><input type="text" name="retainedHours" value="<?= $retainedHours ?>" /> <i>* for Administrative Retainer plans only</i> </td>
	</tr>
	<tr id="hoursPurchased">
		<td>Hours Purchased</td>
		<td style="color:red;font-size:9pt;"><input type="text" name="hoursPurchased" value="<?= $hoursPurchased ?>" /> <i>* for Administrative Prepaid plans only</i> </td>
	</tr>	
	<tr>
		<td>Status:</td>
		<td>
	...
	<tr>
		...
		<td><textarea name="notes" cols="60" rows="5"><?= $notes ?></textarea></td>
	</tr>	
	<tr>
