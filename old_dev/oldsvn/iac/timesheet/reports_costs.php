...
<p class="color_black">Costs / Billable Report</p>
<?
	...
	<div>
			<? while( $row = mysql_fetch_assoc( $db->result['ref'] ) ): ?>
				...
				<tr class="report_row_total">
					...
					<td>$<?= number_format( $total_billable - $total_cost, 2, ".", "," ) ?> (<?= number_format( ($total_billable - $total_cost) / $total_billable * 100, 1, ".", "," ) ?>%)</td>			</tr>
				</tr>
				<?
					$total_cost = 0;
					$total_billable = 0;
				...
				<td>$<?= number_format( $row['rate_billable'] * $row['hours'], 2, ".", "," ) ?></td>
				<? $total_billable += $row['rate_billable'] * $row['hours']; ?>


				<td><strong>$<?= number_format( $total_billable - $total_cost, 2, ".", "," ) ?></strong> (<?= number_format( round( ( ( $row['rate_billable'] - $row['rate'] ) / $row['rate_billable'] ) * 100, 1 ), 1, ".", "," ) ?>%)</td>
...
<? else: ?>
<p class="color_black">Costs / Billable Report</p>
	<form method="post" action="reports.php?report=costs&action=generate">
		<table cellpadding="5" border="0">
			...
			<tr>
				...
				<option value="allopen">All Open Projects</option>
				<option value="allclosed">All Closed Projects</option>
				<option value="">-------------------------</option>
