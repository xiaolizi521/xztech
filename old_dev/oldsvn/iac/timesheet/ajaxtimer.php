			...
			<select id="project" onchange="getTasks();">
				...
				<?	
					if( $employeeArray['type'] == "super-manager" )
						...
						WHERE project.status != \"completed\" AND project.client_id != 0 ORDER BY project.name ASC" );
					else
						$db->query( "SELECT project.name AS name, 
						project.id AS id,
						client.organization AS organization,
						...
						FROM project 
						INNER JOIN client ON project.client_id = client.id
						INNER JOIN project_employees ON project_employees.project_id = project.id WHERE project_employees.employee_id = ".$employeeArray['id']." AND project.status != \"completed\" AND project.client_id != 0 ORDER BY project.name ASC" );
