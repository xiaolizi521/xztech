...
class CreateImage extends CallImage {
	...
	/* Catch any query errors or other errors. */
		...
		$transcolor = imagecolorallocatealpha($this->im,0,0,0,127);
		imagefill($this->im, 0, 0, $transcolor);
		...
		imagegif($this->im2,$this->filename);
	...
	function generate_bg() {
		...
		error_reporting(0);
		if(!imagecopy($this->im2, $this->im, 0, 0, 0, 0, $this->data['width'], $this->data['height'])) {	
			throw new BgFail();
		}
