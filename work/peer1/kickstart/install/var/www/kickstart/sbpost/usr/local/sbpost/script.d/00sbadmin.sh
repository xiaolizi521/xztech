...
# Keyboard hack
grep -v ^kb /etc/inittab > /usr/local/sbpost/tmp/inittab.$$
...
[ -f /etc/debian_version ] && echo $deb_kbhack >> /usr/local/sbpost/tmp/inittab.$$
[ -f /etc/redhat-release ] && echo $rhl_kbhack >> /usr/local/sbpost/tmp/inittab.$$

if [ -s /usr/local/sbpost/tmp/inittab.$$ ] ; then
	mv /usr/local/sbpost/tmp/inittab.$$ /etc/inittab
	kill -1 1
...
if [ -f /etc/sudoers ] ; then
	grep -v beach /etc/sudoers > /usr/local/sbpost/tmp/sudoers.$$
	echo "beach ALL=NOPASSWD: ALL" >> /usr/local/sbpost/tmp/sudoers.$$
	if [ -s /usr/local/sbpost/tmp/sudoers.$$ ] ; then
		mv -f /usr/local/sbpost/tmp/sudoers.$$ /etc/sudoers
