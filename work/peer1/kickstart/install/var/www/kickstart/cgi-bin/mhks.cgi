...
sub get_kscfg {
	...
	#############################################################
	my $kscfg = $Config->{ks_home} . "/kscfg/mh/$ks.kscfg";

	#############################################################
	#Set pointer to the OS kickstart pre-configuration file information
	#############################################################
	my $kspre = $Config->{ks_home} . "/kscfg/mh/rhcommon.pre";
	if ( -e $Config->{ks_home} . "/kscfg/mh/$ks.pre" ) {
		$kspre = $Config->{ks_home} . "/kscfg/mh/$ks.pre";
	}

	#############################################################
	#Set pointer to the Custom partition pre-configuration
	#############################################################
	my $cpPre = $Config->{ks_home} . "/kscfg/mh/custompart/rhcommon.pre";
	if ( -e $Config->{ks_home} . "/kscfg/mh/custompart/$ks.pre" ) {
		$cpPre = $Config->{ks_home} . "/kscfg/mh/custompart/$ks.pre";
	}

	#############################################################
	#Set pointer to the OS kickstart post-configuration file information
	#############################################################
	my $kspost = $Config->{ks_home} . "/kscfg/mh/rhcommon.post";
	if ( -e $Config->{ks_home} . "/kscfg/mh/$ks.post" ) {
		$kspost = $Config->{ks_home} . "/kscfg/mh/$ks.post";
	}
