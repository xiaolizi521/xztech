...
sub sas_check() {
	use strict;
	my $ctl = `/tmp/tw_cli info | grep c[0-9] | awk '{ print \$1 }'`;
	$ctl =~ s/\s+$//;
	my $sas = `/tmp/tw_cli /$ctl show | awk '{ print \$6 }' | grep -om1 SAS`;
	$sas =~ s/\s+$//;
	if ( $sas eq 'SAS') {
		$_[0]->{"sas_drive"} = "1";
		print $_[0]->{"sas_drive"} . "\n";
...
#        print $_[0]->{"3ware"} . "\n";
        ...
        $raid_model =~ s/\s+$//;
        if ($raid_model eq "9690SA-8I") {
                $_[0]->{"raid_controller"} = "9690SA-8I";
                $_[0]->{"raid_controller_hash"} = &raid_hash("9690SA-8I");
        ...
        elsif ($raid_model eq "9690SA-4I") {
                $_[0]->{"raid_controller"} = "9690SA-4I";
                $_[0]->{"raid_controller_hash"} = &raid_hash("9690SA-4I");
