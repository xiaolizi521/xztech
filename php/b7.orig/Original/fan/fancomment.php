...
function CommentOption( $id, $username, $type ) {
    ...
    { $pg = mysql_real_escape_string ( $_GET['pg'] ); }
	if ( $type == 'delete' ) {
		$type_url = '<a href="http://www.bleach7.com?page=fan/fanartview&amp;id=' . $_GET['id'] . '&amp;pg=' . $pg . '&amp;' . $type . '=' . $id . ',' . $username . '" onclick="javascript:DeleteComment (\'http://www.bleach7.com?page=fan/fanartview&amp;id=' . $_GET['id'] . '&amp;pg=' . $pg . '&amp;' . $type . '=' . $id . ',' . $username . '\')">' . ucwords( $type ) . '</a>';
	}elseif ( $type == 'ban' ){

			$type_url = '<a href="http://www.bleach7.com?page=fan/bancomments&amp;user=' . $username . '">' . ucwords( $type ) . '</a>';
	}
	else {
		$type_url = '<a href="http://www.bleach7.com?page=fan/fanartview&amp;id=' . $_GET['id'] . '&amp;pg=' . $pg . '&amp;' . $type . '=' . $id . ',' . $username . '#comment">' . ucwords( $type ) . '</a>';
	}
	return $type_url;
}
...
if ( isset ( $_POST['submit_comment'] ) ) {
	if ( !isbanned ( $user_info['user_id'] ) ) {
		...
		if ( $pages_num == 0 ) {
			header ( 'Location: http://www.bleach7.com?page=fan/fanartview&id=' . $id . '&pg=1' );
		}
		else {
			header ( 'Location: http://www.bleach7.com?page=fan/fanartview&id=' . $id . '&pg=' . $pages_num );
		}
	}
}
...
if ( isset ( $_POST['edit_comment'] ) ) {
	...
	echo "<script>document.location='index.php?page=fan/fanartview&id=$id&pg=$pg'</script>";
...
if ( $comments_num == 0 ) {
	...
	PaginateGallery( 'pg', $pages_num, 'fan/fanartview&amp;id=' . $_GET['id'] );
