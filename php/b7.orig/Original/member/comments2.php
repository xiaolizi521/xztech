...
if ( mysql_num_rows ( $result_news ) == 0 ) {
	...
	while ( $show_news = mysql_fetch_array ( $result_news ) ) {
		...
		$news = htmlentities  ( $news, ENT_QUOTES, ISO-8859-15 );
...
if ( isset ( $_POST['edit_comment'] ) ) {
	...
	$update_comment = mysql_query ( 'UPDATE `news_comments` SET comment=\'' . $comment_post . '\' WHERE `newsid`=\'' . mysql_real_escape_string ( $id ) . '\' AND `id`=\'' . mysql_real_escape_string ( $edit_id ) . '\'' );
	header ( 'Location: ' . $site_path . '/comments&id=' . $id . '&pg=' . $pages_num );
}
...
if ( $comments_num == 0 ) {
	...
	while ( $show_comments = mysql_fetch_array ( $result_comments ) ) {
		...
		$comment_B7 = new B7_User ( $show_comments['user_id'], $show_comments['type'], $show_comments['loyalty'] );
