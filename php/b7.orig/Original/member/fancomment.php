...
function CommentOption( $id, $username, $type ) {
    ...
    { $pg = mysql_real_escape_string ( $_GET['pg'] ); }
	if ( $type == 'delete' ) {
		$type_url = '<a href="'.$site_path.'/fanartview&amp;id=' . $_GET['id'] . '&amp;pg=' . $pg . '&amp;' . $type . '=' . $id . ',' . $username . '" onclick="javascript:DeleteComment (\'' . $site_path . '/fanartview&amp;id=' . $_GET['id'] . '&amp;pg=' . $pg . '&amp;' . $type . '=' . $id . ',' . $username . '\')">' . ucwords( $type ) . '</a>';
	}elseif ( $type == 'ban' ){

			$type_url = '<a href="' . $site_path . '/banfanartview&amp;user=' . $username . '">' . ucwords( $type ) . '</a>';
	}
	else {
		$type_url = '<a href="' . $site_path . '/fanartview&amp;id=' . $_GET['id'] . '&amp;pg=' . $pg . '&amp;' . $type . '=' . $id . ',' . $username . '#comment">' . ucwords( $type ) . '</a>';
	}
	return $type_url;
}
...
if ( isset ( $_POST['submit_comment'] ) ) {
	if ( !isbanned ( $user_info['user_id'] ) ) {
		...
		if ( $pages_num == 0 ) {
			header ( 'Location: http://www.bleach7.com?page=member/fanartview&id=' . $id . '&pg=1' );
		}
		else {
			header ( 'Location: http://www.bleach7.com?page=member/fanartview&id=' . $id . '&pg=' . $pages_num );
		}
	}
}
...
if ( isset ( $_POST['edit_comment'] ) ) {
	$post= mysql_real_escape_string ( htmlspecialchars ( $_POST['comment_post'], ENT_QUOTES ) );
	$edit = $_POST['editid']; 
	list ( $edit_id, $edit_username ) = explode ( ",", $edit );
	$update_post= mysql_query ( "UPDATE gallery_comments SET post='$comment' WHERE newsid='$id' AND id='$edit_id'" );
...
if ( isset ( $_GET['delete'] ) && !empty ( $_GET['delete'] ) ) {
	...
	$result_delete_post = mysql_query ( 'SELECT `gallery_comments`.`id`, `gallery_comments`.`newsid`, `gallery_comments`.`comment`, `users`.`username` FROM `gallery_comments` LEFT JOIN `users` ON ( `gallery_comments`.`poster` = `users`.`username` ) WHERE `gallery_comments`.`newsid` = \'' . mysql_real_escape_string ( $id ) . '\' AND `gallery_comments`.`id` = \'' . mysql_real_escape_string ( $delete_id ) . '\' AND `users`.`username` = \'' . mysql_real_escape_string ( $delete_username ) . '\'' );
	if ( mysql_num_rows ( $result_delete_post ) > 0 ) {
		$delete_post = mysql_query ( 'DELETE FROM `gallery_comments` WHERE `newsid` = \'' . mysql_real_escape_string ( $id ) . '\' AND `id` = \'' . mysql_real_escape_string ( $delete_id ) . '\'' );
...
if ( $comments_num == 0 ) {
	...
	Paginate( 'pg', $pages_num, 'fanartview&amp;id=' . $_GET['id'] );
...
if ( $edit != 'null' && !empty ( $edit ) ) {
	list ( $edit_id, $edit_username ) = explode ( ",", $edit );
	$result_edit_post = mysql_query ( 'SELECT `gallery_comments`.`id`, `gallery_comments`.`pid`, 			`gallery_comments`.`comment`, `users`.`username` FROM `gallery_comments` LEFT JOIN `users` ON ( `gallery_comments`.`poster` = `users`.`username` ) WHERE `gallery_comments`.`newsid` = \'' . mysql_real_escape_string ( $id ) . '\' AND `gallery_comments`.`id` = \'' . mysql_real_escape_string ( $edit_id ) . '\' AND `users`.`username` = \'' . mysql_real_escape_string ( $edit_username ) . '\'' );
...
if ( $quote != 'null' && !empty ( $quote ) ) {
	...
	$result_quote_post = mysql_query ( 'SELECT `gallery_comments`.`id`, `gallery_comments`.`newsid`, `gallery_comments`.`comment`, `users`.`username` FROM `gallery_comments` LEFT JOIN `users` ON ( `gallery_comments`.`poster` = `users`.`username` ) WHERE `gallery_comments`.`newsid` = \'' . mysql_real_escape_string ( $id ) . '\' AND `gallery_comments`.`id` = \'' . mysql_real_escape_string ( $quote_id ) . '\' AND `users`.`username` = \'' . mysql_real_escape_string ( $quote_username ) . '\'' );
