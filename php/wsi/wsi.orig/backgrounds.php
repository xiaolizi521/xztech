...
if ($_POST['submit']) {
	...
	if ($_POST['name']) {
		if ($type == "image/png" || $type == "image/jpeg" || $type == "image/gif" || $type == "image/pjpeg" ) {
			...
			@chmod("users/" . $_SESSION[username],0777);
			@mkdir("users/" . $_SESSION[username] . "/images");
			@chmod("users/" . $_SESSION[username] . "/images",0777);
			...
			elseif ($width > 500 || $height > 100) {
				...
				$path = "users/" . $_SESSION['username'] . "/images/" . $filename . "png" ;
				echo $path;
				imagecopy($im,$im2,0,0,0,0,$width,$height);
				imagepng($im,$path) or die("There was an issue uploading your image. We are looking into it.");
				imagedestroy($im);
				imagedestroy($im2);
...
?>
<form action='' method='post' enctype="multipart/form-data">File
Location: <input type='file' name='file'><br>
